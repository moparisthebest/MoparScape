/* Class68_Sub26 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class68_Sub26 extends Class68 {
    public int anInt3178;
    public int anInt3179;
    public int anInt3180;
    public int anInt3181;
    public static int anInt3182;
    public static int anInt3183;
    public static RSString aRSString_3184
            = Class112.method1668(43, "Ausw-=hlen");
    public Class68_Sub28_Sub3 aClass68_Sub28_Sub3_3185;
    public static int anInt3186;
    public int anInt3187;
    public int anInt3188;
    public int anInt3189;
    public static int anInt3190;
    public static Class21renamed aClass21_3191;
    public Class20 aClass20_3192;
    public int anInt3193;
    public Class68_Sub27 aClass68_Sub27_3194;
    public static RSString aRSString_3195;
    public int anInt3196;
    public int anInt3197;
    public int anInt3198;
    public int anInt3199;
    public static int anInt3200;
    public int anInt3201;
    public int anInt3202;
    public static int anInt3203;
    public Class68_Sub11_Sub1 aClass68_Sub11_Sub1_3204;
    public static int anInt3205;
    public int anInt3206;
    public static int anInt3207;
    public int anInt3208;
    public int anInt3209;

    public void method1214(int i) {
        aClass68_Sub11_Sub1_3204 = null;
        if (i != 2)
            method1217(89);
        anInt3203++;
        aClass20_3192 = null;
        aClass68_Sub28_Sub3_3185 = null;
        aClass68_Sub27_3194 = null;
    }

    public static void method1215(byte i) {
        Class93.aClass98_1662.method1566((byte) 94);
        Class7.aClass98_124.method1566((byte) 94);
        int i_0_ = -72 / ((i - -32) / 57);
        anInt3205++;
        Class96.aClass98_1694.method1566((byte) 94);
    }

    public static void method1216(int i) {
        anInt3190++;
        if (i == 2) {
            if (Class66.aClass123_1200 != null) {
                synchronized (Class66.aClass123_1200) {
                    Class66.aClass123_1200 = null;
                }
            }
        }
    }

    public static void method1217(int i) {
        aRSString_3195 = null;
        if (i != 32125)
            method1215((byte) 77);
        aClass21_3191 = null;
        aRSString_3184 = null;
    }

    static {
        anInt3183 = 2;
        aRSString_3195 = Class112.method1668(43, "(Y");
    }
}
