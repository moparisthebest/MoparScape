...
, int torrent_download_limit)
{
     ...
     if(is_magnet_link){
          boost::optional<std::string> display_name = url_has_argument(link, "dn");
          if (display_name){
               ...
               if(ec){
                    ...
                    printf("parse_magnet_uri NAME: %s\n", name.c_str());
                    // give the path to the possible torrent file
                    torrent = (save_path / (name + ".torrent")).string();
               }
          }
     }else{
          torrent = link;
     ...
     if(!torrent.empty()){
          ...
          if(!ec){
               ...
               std::string filename = (save_path / (name + ".resume")).string();
...
/*          }else{
          ...
          save_file((h.save_path() / name).string() + ".torrent", out);
     ...
     while (num_resume_data > 0)
     {
          ...
          // we have to kludge it here to find OUR name for the torrent
          std::string name;
          for (handles_t::iterator i = handles.begin();
          i != handles.end(); ++i)
          {
               torrent_handle& h2 = i->second;
               if(h == h2){
                    name = i->first;
                    break;
               }
          ...
          save_file((h.save_path() / name).string() + ".resume", out);
     }
