...
, int torrent_download_limit)
{
     ...
     if(is_magnet_link){
          ...
          torrent = (save_path / (hash_to_str(info_hash) + ".torrent")).string();
     ...
     if(!torrent.empty()){
          ...
          if(!ec){
               std::string filename = (save_path / (hash_to_str(t->info_hash()) + ".resume")).string();
     ...
     torrent_handle h;
     if(is_magnet_link)
          h = add_magnet_uri(ses, link.c_str(), p, ec);
     else
          h = ses.add_torrent(p, ec);
     //torrent_handle h = add_magnet_uri(ses, torrent.c_str(), p, ec);
     if (ec)
     {
     ...
     for (std::vector<announce_entry>::iterator i = tr.begin(), end(tr.end()); i != end; ++i)
          printf("TRACKER FOR THIS TORRENT: %s tier %d\n", i->url.c_str(), i->tier);
     }
     ...
     handles.insert(std::pair<const std::string, torrent_handle>(name, h));
}
...
void tag(std::string tag, std::string value, bool human_only){
     std::cout << tag << ": " << value << "\n";
}
...
int main(int argc, char* argv[])
{
     ...
     // start Local Service Discovery
     ses.start_lsd();
     // start UPNP and NATPMP
     libtorrent::upnp* my_upnp = ses.start_upnp();
     libtorrent::natpmp* my_natpmp = ses.start_natpmp();

     for (std::vector<std::string>::iterator i = torrents.begin()
          , end(torrents.end()); i != end; ++i)
     {
          // if it's a torrent file, open it as usual
          add_torrent(ses, handles, i->c_str(), preferred_ratio, allocation_mode, save_path, torrent_upload_limit, torrent_download_limit);
     }
...
/*
*0  ubuntu-10.10-desktop-i386.iso             downloading   down: (0.55kB) up:          (0.43kB) swarm:   23: 125  bw queue: (0|0) all-time (Rx: 15.4MB Tx:       ) seed rank: 0
progress  : 15843328    Bytes   2.18% ####----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
peers: 1 (1026) seeds: 0 distributed copies: 0.02 sparse regions: 28 download:          next announce: 00:29:18 tracker: http://tracker.prq.to/announce
*/
               ...
               if (s.state != torrent_status::queued_for_checking && s.state != torrent_status::checking_files)
               {
                    ...
                    tag("state", (paused && !auto_managed)?"paused":(paused && auto_managed)?"queued":state_str[s.state]);
                    ...
                    tag("bytes",  str);
                    tag("progress_bar", progress_bar(s.progress_ppm / 1000, terminal_width - 43, progress_bar_color), true);
               }
               else
               {
                    ...
                    tag("progress_bar", progress_bar(s.progress_ppm / 1000, terminal_width - 43 - 20, "35"), true);
               }
     ...
     i != handles.end(); ++i)
     {
          ...
          printf("saving torrent file for %s\n", h.name().c_str());
          torrent_info ti = h.get_torrent_info();
          create_torrent new_torrent(ti);
          // sometimes no trackers are added into our torrents, I don't know why, so just put them in there the hard way
//          if(ti.trackers().size() == 0){
          ...
          // otherwise we already have trackers, but some of these are invalid? :(  So fix them
/*          }else{
               std::vector<announce_entry> tr = ti.trackers();
               for (std::vector<announce_entry>::iterator i = tr.begin()
                    , end(tr.end()); i != end; ++i)
               {
                    // if the url isn't long enough, or doesn't start with http or udp, don't add it
                    // I chose 10 because the shortest valid url I could imagine would be something like udp://n.me
                    if( (strlen(i->url.c_str()) < 10) || !( (std::strstr(i->url.c_str(), "http://") == i->url.c_str()) || (std::strstr(i->url.c_str(), "udp://") == i->url.c_str()) ) ){
                         printf("removing tracker from torrent file %s tier %d\n", i->url.c_str(), i->tier);
                         //ti.trackers().erase(i);
                         ti.remove_tracker(i);
                         continue;
                    }
          ...
          save_file((h.save_path() / hash_to_str(h.info_hash())).string() + ".torrent", out);
     ...
     while (num_resume_data > 0)
     {
          ...
          save_file((h.save_path() / hash_to_str(h.info_hash())).string() + ".resume", out);
     }
