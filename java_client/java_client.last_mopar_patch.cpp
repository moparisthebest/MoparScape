	...
	using namespace libtorrent;
     ...
     if(is_magnet_link){
          ...
          if (!btih || (btih->compare(0, 9, "urn:btih:") != 0)){
               debug("invalid magnet link (missing info hash)");
               return std::string();
          }
          sha1_hash info_hash;
     ...
     debug("torrent '%s', link '%s'", torrent.c_str(), link.c_str());
     ...
     std::vector<char> buf;
     if(!torrent.empty()){
          debug("torrent we are trying to open: '%s'", torrent.c_str());
          ...
          if(!ec){
               ...
               p.url.clear();
               debug("USING torrent file: %s", torrent.c_str());
          }else if(!is_magnet_link){
               debug("%s: %s", torrent.c_str(), ec.message().c_str());
               return std::string();
          }
     }
...
int main(int argc, char* argv[])
{
	...
	//std::vector<std::string> torrents;
     ...
     for (int i = 1; i < argc; ++i)
     {
          if (argv[i][0] != '-')
          {
               ...
               if (argc == i || argv[i][0] == '-'){
                    printf("Fatal error: SAVE_PATH must always follow each TORRENT|MAGNETURL|URL.\n");
                    return 1;
               }
		...
		switch (argv[i][1])
		{
               ...
               case 's':
                    if(i != 1 && (i != 2 && print_debug)){
                         printf("Fatal error: -s not first argument, if you specify it, it must be first (or second to -b).\n");
                         return 1;
                    }
			...
			case 'O': settings.allow_reordered_disk_operations = false; --i; break;
               ...
               case 'b':
                    if(i != 1){
                         printf("Fatal error: -b not first argument, if you specify it, it must be first.\n");
                         return 1;
                    }
			...
			case 'P':
				{
					...
					if (port == 0)
					{
						debug("invalid proxy hostname, no port found");
						break;
					}
					*port++ = 0;
					ps.hostname = arg;
			...
			case 'L':
				{
					...
					if (pw == 0)
					{
                              debug("invalid proxy username and password specified");
						break;
					}
					*pw++ = 0;
					ps.username = arg;
	...
	ses.set_proxy(ps);
     ...
     if(listen_port == -1) listen_port = (rand()%65525)+1024;
	...
	while (true)
	{
		...
		while (sleep_and_input(&c, refresh_delay))
		{
			...
			if (c == 't') print_trackers = !print_trackers;
               if (c == 'h') print_dht_status = !print_dht_status;
               if (c == 'u') print_utp_stats = !print_utp_stats;
               if (c == 'p') print_alerts = !print_alerts;
               if (c == 's') share_mode = !share_mode;
               if (c == 'c') constant_loop = !constant_loop;
		}
		if (c == 'q') break;
